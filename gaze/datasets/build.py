...
def get_loader(name: str, root_dir: str, random_flag=False,depth_on=False,x_loss=False,input_size=224, output_size=64, batch_size=48, num_workers=6, is_train=True,is_subsample_test_set=True,gaze_point_threshold=0,sigma=3):
    if name == "gazefollow":# we enter into here
        ...
        dataset = GazeFollow(root_dir, labels, random_size=random_flag,depth_on=depth_on,x_loss=x_loss,input_size=input_size, output_size=output_size, is_test_set=not is_train,is_subsample_test_set=is_subsample_test_set,gaze_point_threshold=gaze_point_threshold,sigma=3)
        loader = DataLoader(
        ...
        dataset = VideoAttentionTargetImages(
            root_dir, labels, random_size=random_flag,depth_on=depth_on,input_size=input_size, output_size=output_size, is_test_set=not is_train,sigma=3
        )
        loader = DataLoader(
            dataset=dataset, batch_size=batch_size, shuffle=is_train, num_workers=num_workers, pin_memory=True
        )
        ...
        dataset = MHUGImages(
            root_dir, labels, random_size=random_flag,depth_on=depth_on,input_size=input_size, output_size=output_size,sigma=3
        )
        loader = DataLoader(
            dataset=dataset, batch_size=batch_size, shuffle=is_train, num_workers=num_workers, pin_memory=True
        )
