...
def plottings(schedular_name,step):
    ...
    data_ = torch.zeros([65,65])
    data_set_size=0
    data_set_size_2=0
    # step=1
    for batch in range(n_iter):
        data_source = next(the_source)
        (
                s_rgb,
                s_heads,# the face
                s_masks,
        ...
        t=torch.full((gaze_points.shape[0],),999).long()
        heatmap_new = normalize_to_neg_value_to_value(gaze_points.squeeze(1),norm)
        ...
        # print(noise.shape)
        # print(sqrt_one_minus_alphas_cumprod.shape)
        # print(sqrt_alphas_cumprod.shape)
        # print(t.shape)

        x = q_sample(heatmap_new,
                 t,
                 sqrt_alphas_cumprod,
                 sqrt_one_minus_alphas_cumprod, 
                 noise = noise
                 )
        # x = torch.clamp(x, min=-1 * norm, max=norm)
        x = wrap_clamp_tensor(x, -1*norm, norm)

        # print(x)
        x = unnormalize_to_neg_value_to_value(x,norm)
        data_set_size+=1
        data_set_size_2+=x.shape[0]
        new_list =((x.squeeze(1)*64)).long()
        unique_indices, counts = torch.unique(new_list, return_counts=True, dim=0)
        data_[unique_indices[:, 0], unique_indices[:, 1]] += counts
        # new_list =((gaze_points.squeeze(1)*64)).int().tolist()
        # for i in range(len(new_list)):
        #     data_[new_list[i]]+=1
    print(data_set_size)
    print(data_set_size_2)
    values = data_.int().tolist()
    print(torch.sum(data_))
    file_name = "out_noised_step_999_01v2.csv"
